...
const { notifyMany, sendNotification } = require("../utils/notification");
...
exports.submitContactForm = async (req, res) => {
  ...
  try {
    ...
    // Notify admins of the new issue/contact
    const admins = await User.find({ role: "admin" }).select("_id");
    if (admins.length > 0) {
      const adminIds = admins.map(a => a._id);
      await notifyMany(adminIds, {
        title: `New ${type === "complaint" ? "Issue" : "Contact"} Submitted`,
        message: `${name} has submitted a new ${type || "general"} query.`,
        type: "system",
  ...
  try {
    ...
    if (user) {
      sendNotification(user._id, {
        ...
        message: "An admin has responded to your inquiry.",
        type: "system",
        relatedId: contact._id,
        relatedModel: "Contact"
      });
  ...
  try {
    ...
    if (user) {
      sendNotification(user._id, {
        ...
        message: "Your complaint has been successfully resolved by our team.",
        type: "system",
        relatedId: contact._id,
        relatedModel: "Contact"
      });
