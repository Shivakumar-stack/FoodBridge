...
async function runTests() {
  try {
    ...
    const volunteerReg = await makeRequest(
      "POST",
      "/auth/register",
      volunteerData,
    );
    ...
    const donationRes = await makeRequest(
      "POST",
      "/donations",
      donationData,
      donorToken,
    ...
    const availableDonations = await makeRequest(
      "GET",
      "/donations/volunteer/available",
      null,
      volunteerToken,
    );
    console.log(
      "Found Donations:",
      availableDonations.data ? availableDonations.data.length : "none",
    );
    ...
    const volunteerDashboard = await makeRequest(
      "GET",
      `/auth/dashboard`,
      null,
      volunteerToken,
    ...
    console.log(
      "Volunteer Dashboard Success:",
      volunteerDashboard.success,
      volunteerDashboard.data.stats,
    );

    // 9. Test Donor Dashboard
    console.log("Testing Donor Dashboard Analytics...");
    const donorDashboard = await makeRequest(
      "GET",
      `/auth/dashboard`,
      null,
      donorToken,
    ...
    console.log(
      "Donor Dashboard Success:",
      donorDashboard.success,
      donorDashboard.data.stats,
    );
