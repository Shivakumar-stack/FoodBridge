...
async function test() {
  ...
  try {
    ...
    console.log(
      `   Status: ${signupRes.status} | Success: ${signupData.success}`,
    );
  ...
  try {
    ...
    console.log(
      `   Status: ${loginRes.status} | Success: ${loginData.success}`,
    );
  ...
  if (!token) {
    ...
    try {
      ...
      console.log(`   ⚠️ Secondary login attempt failed: ${e.message}`);
    }
  ...
  if (authToken) {
    try {
      ...
      const data = await res.json();
      console.log(
        `   Status: ${res.status} | Success: ${data.success} | Count: ${data.count || "N/A"}`,
      );
  ...
  if (adminToken || token2) {
    try {
      ...
      if (data.success) {
        ...
        console.log(
          `   Metrics: deliveries=${data.data.metrics.deliveriesCompleted}, food=${data.data.metrics.totalFoodQuantity}`,
        );
      } else {
  ...
  try {
    ...
    const data = await res.json();
    console.log(
      `   Status: ${res.status} | Success: ${data.success} | Count: ${data.count}`,
    );
