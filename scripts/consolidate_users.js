...
async function main() {
    ...
    if (!keepIds.has(d.donor_id?.toString())) {
      ...
      await db
        ...
        .updateOne(
          ...
          {
            $set: {
              donor_id: newDonor._id,
              donorName:
                newDonor.name || `${newDonor.firstName} ${newDonor.lastName}`,
            },
          },
        );
      fixed++;
  ...
  for (let i = 0; i < volunteers.length; i++) {
    ...
    if (!keepIds.has(v.user_id?.toString())) {
      ...
      await db
        ...
        .updateOne({ _id: v._id }, { $set: { user_id: newUser } });
      fixed++;
    }
  }
  ...
  for (let i = 0; i < deliveries.length; i++) {
    ...
    if (!keepIds.has(d.ngo_id?.toString())) {
      ...
      await db
        ...
        .updateOne({ _id: d._id }, { $set: { ngo_id: newNgo } });
      fixed++;
    }
  }
  ...
  for (let i = 0; i < mealservers.length; i++) {
    ...
    if (!keepIds.has(m.ngo_id?.toString())) {
      ...
      await db
        ...
        .updateOne({ _id: m._id }, { $set: { ngo_id: newNgo } });
      fixed++;
    }
  }
  ...
  for (let i = 0; i < invLogs.length; i++) {
    ...
    if (inv.loggedBy && !keepIds.has(inv.loggedBy.toString())) {
      ...
      await db
        ...
        .updateOne({ _id: inv._id }, { $set: { loggedBy: newNgo } });
      fixed++;
    }
  }
  ...
  for (const n of notifications) {
    if (n.user && !keepIds.has(n.user.toString())) {
      await db
        ...
        .updateOne({ _id: n._id }, { $set: { user: keepDonorIds[0] } });
      fixed++;
    }
  }
  ...
  console.log(
    broken === 0
      ? "  ✅ All donations → valid users"
      : `  ❌ ${broken} broken donation refs`,
  );
  ...
  console.log(
    broken === 0
      ? "  ✅ All deliveries → valid donations"
      : `  ❌ ${broken} broken delivery refs`,
  );
  ...
  for (const d of await db
    .collection("deliveries")
    .find({}, { projection: { volunteer_id: 1 } })
    .toArray()) {
    if (!volunteerIds.has(d.volunteer_id?.toString())) broken++;
  }
  console.log(
    broken === 0
      ? "  ✅ All deliveries → valid volunteers"
      : `  ❌ ${broken} broken delivery vol refs`,
  );
  ...
  console.log(
    broken === 0
      ? "  ✅ All volunteers → valid users"
      : `  ❌ ${broken} broken volunteer refs`,
  );
  ...
  console.log(
    broken === 0
      ? "  ✅ All inventorylogs → valid donations"
      : `  ❌ ${broken} broken invlog refs`,
  );
