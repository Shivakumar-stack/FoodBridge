...
async function validateAndFixReferences(db) {
  ...
  const mealserverNgoIds = new Set(
    mealservers.map((m) => m.ngo_id?.toString()).filter(Boolean),
  );
  ...
  for (const d of donations) {
    ...
    if (!donorId || !userIds.has(donorId)) {
      ...
      await db
        ...
        .updateOne({ _id: d._id }, { $set: { donor_id: randomUser._id } });
      fixCount++;
    }
  }
  if (fixCount > 0) {
  ...
  for (const d of donations) {
    ...
    if (volId && !volunteerIds.has(volId)) {
      const randomVol =
        validVolunteerIdsList[
          Math.floor(Math.random() * validVolunteerIdsList.length)
        ];
      await db
        .collection("donations")
        .updateOne(
          ...
          {
            $set: {
              assigned_volunteer: new mongoose.Types.ObjectId(randomVol),
            },
          },
        );
      fixCount++;
  ...
  for (const del of deliveries) {
    ...
    if (!donId || !donationIds.has(donId)) {
      const randomDon =
        validDonationIdsList[
          Math.floor(Math.random() * validDonationIdsList.length)
        ];
      await db
  ...
  for (const del of deliveries) {
    ...
    if (!ngoId || (!userIds.has(ngoId) && !mealserverNgoIds.has(ngoId))) {
      const randomNgo =
        validNgoIdsList[Math.floor(Math.random() * validNgoIdsList.length)];
      await db
        .collection("deliveries")
        .updateOne({ _id: del._id }, { $set: { ngo_id: randomNgo } });
      fixCount++;
    }
  }
  if (fixCount > 0) {
  ...
  for (const v of volunteers) {
    ...
    if (!uid || !userIds.has(uid)) {
      ...
      await db
        ...
        .updateOne({ _id: v._id }, { $set: { user_id: randomUser._id } });
      fixCount++;
    }
  }
  if (fixCount > 0) {
...
async function reduceUsers(db) {
  ...
  const donations = await db
    .collection("donations")
    .find({}, { projection: { donor_id: 1 } })
    .toArray();
  donations.forEach((d) => {
    if (d.donor_id) referencedUserIds.add(d.donor_id.toString());
  });
  ...
  volunteers.forEach((v) => {
    if (v.user_id) referencedUserIds.add(v.user_id.toString());
  });

  // From deliveries.ngo_id and volunteer_id (which may be user IDs)
  const deliveries = await db
    .collection("deliveries")
    .find({}, { projection: { ngo_id: 1 } })
    .toArray();
  deliveries.forEach((d) => {
    if (d.ngo_id) referencedUserIds.add(d.ngo_id.toString());
  });

  // From mealservers.ngo_id
  const mealservers = await db
    .collection("mealservers")
    .find({}, { projection: { ngo_id: 1 } })
    .toArray();
  mealservers.forEach((m) => {
    if (m.ngo_id) referencedUserIds.add(m.ngo_id.toString());
  });

  // From inventorylogs.loggedBy
  const invLogs = await db
    .collection("inventorylogs")
    .find({}, { projection: { loggedBy: 1 } })
    .toArray();
  invLogs.forEach((inv) => {
    if (inv.loggedBy) referencedUserIds.add(inv.loggedBy.toString());
  });
  ...
  notifications.forEach((n) => {
    if (n.user) referencedUserIds.add(n.user.toString());
  });
  ...
  const referencedUsers = allUsers.filter((u) =>
    referencedUserIds.has(u._id.toString()),
  );
  const unreferencedUsers = allUsers.filter(
    (u) => !referencedUserIds.has(u._id.toString()),
  );
  ...
  if (referencedUsers.length >= TARGET_MAX_USERS) {
    ...
    log(
      `  Referenced users (${referencedUsers.length}) >= target (${TARGET_MAX_USERS}), deleting all ${unreferencedUsers.length} unreferenced.`,
    );
  } else {
    ...
    log(
      `  Keeping ${keepFromUnreferenced.length} unreferenced users, deleting ${toDelete.length}.`,
    );
  }
...
async function fixDonationSchema(db) {
  ...
  if (withFoodItems.length > 0) {
    ...
    logWarn(
      `Converted ${withFoodItems.length} donations from foodItems → items`,
    );
    report.schemaFixes.push(
      `${withFoodItems.length} donations: foodItems → items`,
    );
  } else {
  ...
  if (missingItems > 0) {
    ...
    report.schemaFixes.push(
      `${missingItems} donations with empty items (left as-is)`,
    );
  } else {
...
async function finalValidation(db) {
  ...
  if (broken > 0) {
    logErr(`${broken} donations reference non-existent users`);
  ...
  const deliveries = await db
    .collection("deliveries")
    .find({}, { projection: { donation_id: 1, volunteer_id: 1 } })
    .toArray();
  for (const d of deliveries) {
    if (!d.donation_id || !donationIds.has(d.donation_id.toString())) broken++;
  }
  if (broken > 0) {
    logErr(`${broken} deliveries reference non-existent donations`);
    report.validationPassed = false;
  ...
  broken = 0;
  for (const d of deliveries) {
    if (!d.volunteer_id || !volunteerIds.has(d.volunteer_id.toString()))
      broken++;
  }
  if (broken > 0) {
    logErr(`${broken} deliveries reference non-existent volunteers`);
    report.validationPassed = false;
  ...
  const invLogs = await db
    .collection("inventorylogs")
    .find({}, { projection: { donation_id: 1 } })
    .toArray();
  for (const inv of invLogs) {
    if (!inv.donation_id || !donationIds.has(inv.donation_id.toString()))
      broken++;
  }
  if (broken > 0) {
    logErr(`${broken} inventorylogs reference non-existent donations`);
    report.validationPassed = false;
  ...
  if (broken > 0) {
    logErr(`${broken} volunteers reference non-existent users`);
    report.validationPassed = false;
...
async function generateReport(db) {
  ...
  for (const [name, count] of Object.entries(report.initialCounts)) {
    console.log(
      `║   ${name.padEnd(20)} ${String(count).padStart(6)}                           ║`,
    );
  }
  ...
  for (const [name, count] of Object.entries(report.finalCounts)) {
    ...
    console.log(
      `║   ${name.padEnd(20)} ${String(count).padStart(6)}${diffStr.padEnd(17)}║`,
    );
  }
  ...
  console.log("╠══════════════════════════════════════════════════════════╣");
  const status = report.validationPassed
