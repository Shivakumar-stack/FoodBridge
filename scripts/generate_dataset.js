...
function generateUsers() {
  let csv =
    "_id,email,password,role,firstName,lastName,organizationName,city,phone\n";
...
function generateDonations() {
  let csv =
    "_id,donor_id,donorName,address,city,state,zip,status,priority,notes,itemName,category,quantity,unit\n";
  for (let i = 0; i < RECORDS_COUNT; i++) {
...
function generateMealServers() {
  let csv =
    "_id,ngo_id,organization_name,contact_person,phone,city,address,capacity,mealsServedDaily\n";
  for (let i = 0; i < RECORDS_COUNT; i++) {
...
function generateDeliveries() {
  let csv =
    "delivery_id,donation_id,volunteer_id,ngo_id,delivery_status,pickup_time,delivery_time,deliveryNotes\n";
  for (let i = 0; i < RECORDS_COUNT; i++) {
...
function generateInventoryLogs() {
  let csv =
    "mealServer,donation_id,itemName,category,quantity,unit,operationType,loggedBy,city\n";
  for (let i = 0; i < RECORDS_COUNT; i++) {
