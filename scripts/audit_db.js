...
async function run() {
  ...
  for (const name of expected) {
    ...
    console.log(
      `  ${exists ? "✅" : "❌"} ${name}: ${exists ? "EXISTS" : "MISSING"}`,
    );
  }
  ...
  if (sampleDonation) {
    ...
    console.log(
      "  items:",
      Array.isArray(sampleDonation.items)
        ? `✅ (${sampleDonation.items.length} items)`
        : "❌",
    );
    ...
    console.log(
      "  pickup_datetime:",
      sampleDonation.pickup_datetime ? "✅" : "❌",
    );
    console.log(
      "  status:",
      sampleDonation.status ? `✅ (${sampleDonation.status})` : "❌",
    );
    console.log(
      "  assigned_volunteer:",
      sampleDonation.assigned_volunteer !== undefined
        ? "✅"
        : "⚠️ (field missing, ok if null)",
    );
  } else {
  ...
  const donationDonorIds = await db
  ...
  console.log(
    `donations.donor_id → users: ${brokenDonorRefs === 0 ? "✅ All valid" : `❌ ${brokenDonorRefs} broken refs`}`,
  );

  // Check deliveries.donation_id → donations
  const deliveryDonationIds = await db
    .collection("deliveries")
    .distinct("donation_id");
  ...
  console.log(
    `deliveries.donation_id → donations: ${brokenDeliveryDonRefs === 0 ? "✅ All valid" : `❌ ${brokenDeliveryDonRefs} broken refs`}`,
  );

  // Check deliveries.volunteer_id → volunteers or users
  const deliveryVolIds = await db
    .collection("deliveries")
  ...
  console.log(
    `deliveries.volunteer_id → volunteers/users: ${brokenDeliveryVolRefs === 0 ? "✅ All valid" : `❌ ${brokenDeliveryVolRefs} broken refs`}`,
  );
  ...
  console.log(
    `deliveries.ngo_id → users: ${brokenDeliveryNgoRefs === 0 ? "✅ All valid" : `❌ ${brokenDeliveryNgoRefs} broken refs`}`,
  );

  // Check inventorylogs.donation_id → donations
  const invDonIds = await db
  ...
  console.log(
    `inventorylogs.donation_id → donations: ${brokenInvRefs === 0 ? "✅ All valid" : `❌ ${brokenInvRefs} broken refs`}`,
  );
  ...
  console.log(
    `volunteers.user_id → users: ${brokenVolRefs === 0 ? "✅ All valid" : `❌ ${brokenVolRefs} broken refs`}`,
  );
  ...
  console.log(
    `mealservers.ngo_id → users: ${brokenMsRefs === 0 ? "✅ All valid" : `❌ ${brokenMsRefs} broken refs`}`,
  );
