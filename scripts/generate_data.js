...
function generateDonations() {
  let csv =
    "_id,donor_id,donorName,address,city,state,zip,status,priority,notes\n";
  for (let i = 1; i <= BATCH_SIZE; i++) {
...
function generateMealServers() {
  let csv =
    "_id,ngo_id,organization_name,contact_person,phone,city,address,capacity,mealsServedDaily,active\n";
...
function generateInventoryLogs() {
  let csv =
    "mealServer,donation_id,itemName,category,quantity,unit,operationType,loggedBy,city,notes\n";
  for (let i = 1; i <= BATCH_SIZE; i++) {
