...
async function main() {
  ...
  try {
    ...
    // Hard Offline Blocking
    document.querySelectorAll('button[type="submit"]').forEach(btn => {
      btn.disabled = true;
      btn.title = "Form submission disabled (Offline)";
      
      if (!btn.textContent.includes("(Offline)")) {
        const offlineSpan = document.createElement("span");
        offlineSpan.textContent = " (Offline)";
        offlineSpan.style.opacity = "0.8";
        offlineSpan.style.marginLeft = "4px";
        btn.appendChild(offlineSpan);
      }
    });
  }
}
