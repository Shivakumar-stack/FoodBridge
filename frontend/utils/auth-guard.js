...
const AuthGuard = {
  protectedRoutes: [
    {
      path: '/pages/dashboard-unified.html',
      mode: 'redirect',
      redirect: '../pages/login.html',
      message: 'Please log in to view your dashboard'
    },
    {
      path: '/pages/volunteer.html',
      mode: 'redirect',
      redirect: '../pages/login.html',
  ...
  redirectToLogin(message = 'Please log in to continue') {
    ...
    window.location.href = `../pages/login.html?message=${encodeURIComponent(message)}`;
  },
  ...
  checkCurrentRoute() {
      ...
      if (Array.isArray(route.allowedRoles) && route.allowedRoles.length > 0) {
        ...
        if (!allowed) {
          if (route.mode === 'redirect') {
            window.location.href = `../pages/dashboard-unified.html?message=${encodeURIComponent('You are not authorized to access that page.')}`;
            return false;
          }
    ...
    if (route.mode === 'redirect') {
      window.location.href = `${route.redirect || '../pages/login.html'}?message=${encodeURIComponent(route.message || 'Please log in to continue')}`;
      return false;
    }
   ...
   */
  handlePostLoginRedirect(defaultPath = '../pages/dashboard-unified.html') {
...
window.addEventListener('pageshow', (e) => {
  if (e.persisted) {
    try {
      AuthGuard.checkCurrentRoute();
    } catch (err) {
      console.warn('AuthGuard pageshow re-check failed:', err);
    }
  }
});
