...
document.addEventListener("DOMContentLoaded", async () => {
  if (
    ...
    typeof donationService === "undefined"
  ) {
  ...
  document
    .getElementById("refreshDonationsBtn")
    ?.addEventListener("click", () => {
      loadDonations();
    });

  document
    .getElementById("donationsSearchInput")
    ?.addEventListener("input", () => {
      renderDonations();
    });

  document
    .getElementById("donationsStatusFilter")
...
async function loadDonations() {
  ...
  try {
    ...
    list.innerHTML =
      '<li class="pending-empty">Unable to load donations right now.</li>';
  }
}
...
function renderDonations() {
  ...
  const searchTerm = String(
    document.getElementById("donationsSearchInput")?.value || "",
  )
    .trim()
    .toLowerCase();
  const statusFilter = String(
    document.getElementById("donationsStatusFilter")?.value || "all",
  );
  ...
  const filtered = donationsState.items.filter((donation) => {
    ...
    const matchesSearch =
      !searchTerm || itemName.includes(searchTerm) || city.includes(searchTerm);
  ...
  if (!filtered.length) {
    list.innerHTML =
      '<li class="pending-empty">No donations match your filters.</li>';
    return;
  }
  ...
  list.innerHTML = filtered
    .map((donation) => {
      ...
      const quantity = escapeHtml(
        donation?.foodItems?.[0]?.quantity || "Not specified",
      );
